...
class MODISCI:
    ...
    def __init__(
        ...
        if username is None or password is None:
            if not "ORNLDAAC_USERNAME" in os.environ or not "ORNLDAAC_PASSWORD" in os.environ:
                raise RuntimeError("Missing environment variable 'ORNLDAAC_USERNAME' or 'ORNLDAAC_PASSWORD'")

            username = os.environ["ORNLDAAC_USERNAME"]
            password = os.environ["ORNLDAAC_PASSWORD"]
    ...
    def download(self) -> str:
        if exists(self.filename):
            self.logger.info(f"file already downloaded: {cl.file(self.filename)}")
